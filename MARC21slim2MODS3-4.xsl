<xsl:stylesheet xmlns:mods="http://www.loc.gov/mods/v3" xmlns:marc="http://www.loc.gov/MARC21/slim"
	...
	exclude-result-prefixes="xs xlink marc" version="2.0">
...
UVA Revision 1.707 - Transformation can now handle variable for identifers.  Expectation that 
UVA Revision 1.706 - All MODS records will now have <physicalDescription><digitalOrigin>reformatted digital</digitalOrigin></physicalDescription>.  See line ~ 988
UVA Revision 1.705 - Added mods namespace to all tags.
UVA Revision 1.704 - Really dropped <note> for 510 which LC claims to have done at revision 1.40, but it seems that they didn't
UVA Revision 1.703 - Added <xsl:template name="createHoldingSimpleFrom999">.  This template will produce code that can hold "copy information"
in the following manner:

<mods:location>
	<mods:holdingSimple>
		<mods:copyInformation>
		<mods:subLocation>SPEC-COLL</mods:subLocation>
		<mods:shelfLocator>Gordon 1569 .D83</mods:shelfLocator>
		<mods:note displayLabel="barcode">X123456789</mods:note>
		</mods:copyInformation>
	</mods:holdingSimple>       
</mods:location>

This requires a parameter for barcode to be passed to the transformation.  This barcode can be pulled from Tracksys Bibl objects. - 4/21/2011

UVA Revision 1.702 - Added xmlns:mods namespace so output is namespaced properly.
UVA Revision 1.701 -  aec6v and lsc6v looked at the MODS descMetadata coming out of this stylesheet and determine that it is not transforming
the standard call numbers.  Create two new templates to transform 090 and 099 called "createClassificationFrom090" and "createClassificationFrom099".
Both say that the authority for said call number is lc.- 3/1/2010
...
Revision 1.42 - Changed 856 second indicator=1 to go to <mods:location><url displayLabel=”electronic resource”> instead of to <relatedItem type=”otherVersion”><url> 2009/11/20 tmee
...
Revision 1.21 - Mapped 856 ind2=1 or ind2=2 to <relatedItem><mods:location><url>  2008/07/03 tmee
...
Revision 1.02 - Added Log Comment  2003/03/24 19:37:42  ckeith
	...
	<!-- This parameter will be used to pull copy information from local 999 records to create a holdingSimple record for the particular 
	copy in question. -->
	...
	<xsl:template name="marcRecord">
		...
		<xsl:variable name="physicalDescription">
			...
			<!-- Always will have the record be reformatted digital 
		...
		<!-- Comment out creating subject headings from MARC 043
	...
	<!-- Comment out creating subject heading from this MARC field.
	...
	<xsl:template name="createClassificationFrom090">
		<mods:classification authority="dlc">
			<xsl:call-template name="xxx880"/>
			<xsl:call-template name="subfieldSelect">
				<xsl:with-param name="codes">ab</xsl:with-param>
			</xsl:call-template>
		...
		<mods:location>
			<xsl:if test="marc:subfield[@code='a' or @code='b' or @code='e']">
				<mods:physicalLocation>
					<xsl:choose>
						<xsl:when test="contains(marc:subfield[@code='b']/text(), 'SPEC-COLL')">
	...
	<xsl:template name="createHoldingSimpleFrom999">
		<xsl:if test="marc:subfield[@code='i'] = $barcode">
			<mods:location>
				<mods:holdingSimple>
					<mods:copyInformation>
						<mods:subLocation>
						...
						<mods:note type="call_number">
							<xsl:value-of select="marc:subfield[@code='a']/text()"/>
						</mods:note>
						<mods:note type="call_number_sort">
							<xsl:value-of select="marc:subfield[@code='w']/text()"/>
						</mods:note>
						<mods:note type="barcode">
